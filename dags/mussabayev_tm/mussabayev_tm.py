...
with dag:
    ...
    topic_modelling_operator = DjangoOperator(
        ...
        op_kwargs={
            "name": "test",
            "d1": 0.75, # Максимальное допустимое расстояние между всеми возможными попарными комбинациями объектов в составе формируемых сгустков (нечётких протокластеров)
            "d2": 0.75, # Радиус окрестности в пределах которой осуществляется поиск сгустков
            "d3": 0.4, # Минимально допустимая доля общности составов двух произвольных сгустков для установления связи между ними
            "min_clot_size": 20, # Минимально допустимое размер сгустка (количество объектов в сгустке). Сгустки меньшего размера удаляются
