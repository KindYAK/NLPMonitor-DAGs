...
def scrap_youtube_async(yt_api, account, auth_account, datetime_last=None):
    ...
    async def iterator():
        ...
        while True:
            try:
                posts = yt_api.search(channel_id=account.account_id, max_results=100, order_by='date')
    ...
    def document_handler(account, message):
        ...
        comments_list = list()
        if message['video_comment_count']:
            comments = yt_api.get_video_comments(video_id=message['video_id'], max_results=100)  # list of dicts
        ...
        return create_document(
            ...
            text=message['video_description'],
            datetime=date_getter(message),
            num_comments=video_meta['video_comment_count'],
            num_likes=message['video_like_count'],
            num_views=message['video_view_count'],
    ...
    def document_updater(account, message):
        ...
        if message['video_comment_count'] > d.num_comments:
            comments_to_add = message['video_comment_count'] - d.num_comments
        ...
        d.num_likes = message['video_like_count'],
        d.num_views = message['video_view_count']
        d.save()
